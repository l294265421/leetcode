 ...
 * （实际上就是一步步从升序排列到降序排列的过程）：  我们可以这样划分
 * 数组中的元素(a1...|...an),|从an前面移动到 a1前面，每一次，
 * |后面的元素集合都必须经历从升序排列到降序排列的过程，  当|后面的
 * 元素变成降序排列时，这个子集变为最大，需要向前进位（类似十进制进位），
 * 进位的方式就是：交换|后面的所有元素中比|前面一个元素大的最小元素，
 * |后面的元素变为升序，这样就得到了下一个元素，然后，|后面的元素又
 * 需要再次经历从升序排列到降序排列的过程（类比十进制进位，进位之后，
 * 又从0开始），这时，这个子集（|后面的元素组成的集合）的元素个数没变，
 * 但是元素有所变化；而对在对整个升序数组一步步变为降序数组的过程中遇到
 * 的所有升序子数组，也需要经历同样的过程。
 * 
 * 2.也可以这样来理解实现全排列的过程：在数组中，从右至左，数组中的每
 * 一个元素位置都经历这样一个过程，从最初的元素一步步变为它后面的元素
 * 中的最大元素，而每一次变化，它后面的元素集合都需要经历从升序排列到
 * 降序排列的完整过程。
 * 
 * 3.现在又来思考，这种实现方式如何对等我们在数学中学习的，第一个位置
 * 有n种选择，第二个位置有n-1个选择，最后一个位置只有一个选择，这样的
 * 方式呢？我们通过第二点的简化理解可以看到，第一个位置确实有n个选择，
 * 第二个位置有n-1个选择，最后一个位置只有一个选择。（从宏观上，我算
 * 是理解了，但是具体到细节里，我也想象不出）
 * 
 * 4.在完成全排列的过程中，我注意到这样一个事实：数组在变大，变大是通过
 * 进位来实现的；进位作用在这样一个元素身上：它后面的元素是降序排列的，
 * 它及它后面的元素不是降序排列的；类比于从十进制从0变大某个数）
 * @author yuncong
 *
 */
