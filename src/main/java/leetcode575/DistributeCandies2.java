...
public class DistributeCandies2 {
	/**
	 * 
	 * 方案一：给brother每种糖果分一个，少了，就从剩余的糖果里面拿，多了就拿出来
	 * @param candies
	 * @return
	 */
    public int distributeCandies(int[] candies) {
    	int half = candies.length / 2;
    	Set<Integer> distinctCandy = new HashSet<>();
    	for (int candy : candies) {
			distinctCandy.add(candy);
			if (distinctCandy.size() == half) {
				return half;
			}
		}
    	return distinctCandy.size();
    }
}
